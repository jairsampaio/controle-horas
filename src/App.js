...
const App = () => {
  ...
  useEffect(() => {
    ...
    if (isAdmin) {
      tabsPermitidas.push("reports", "team");
    }
  ...
  }, [activeTab, userRole, isAdmin, isSuperAdmin]);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (
        ...
        !excelMenuRef.current.contains(event.target)
      ) {
  ...
  const carregarConfiguracao = useCallback(async () => {
    ...
    try {
      ...
      if (data) {
        const valorConf = data.find(
          (config) => config.chave === "valor_hora_padrao"
        );

        const nomeConf = data.find(
          (config) => config.chave === "nome_consultor"
        );

        if (valorConf) {
          setValorHoraPadrao(valorConf.valor);
        }

        if (nomeConf) {
          setNomeConsultor(nomeConf.valor);
        }
      }
    } catch (error) {
      console.error("Erro config:", error);
    ...
    try {
      ...
      const roleEfetiva =
        userProfile.role === "super_admin"
          ? "super_admin"
          : ["admin", "dono", "gp"].includes(userProfile.role)
          ? userProfile.role
          : ["admin", "dono", "super_admin", "gp"].includes(userProfile.cargo)
          ? userProfile.cargo
          : userProfile.role || userProfile.cargo || "consultor";
      ...
      const podeGerenciarOperacao = [
        "admin",
        "dono",
        "super_admin",
        "gp",
      ].includes(roleEfetiva);

      if (!podeGerenciarOperacao) {
        query = query.eq("user_id", session.user.id);
      }

      const { data: dataServicos, error: errorServicos } = await query.order(
        "data",
        {
          ascending: false,
        }
      ...
      const servicosSeguros =
        roleEfetiva === "gp"
          ? (dataServicos || []).map((servico) => {
              ...
              delete dadosOperacionais.valor_hora;
              delete dadosOperacionais.valor_total;

              return dadosOperacionais;
            })
          : dataServicos || [];

      setServicos(servicosSeguros);
      ...
      if (dataClientes && dataClientes.length > 0) {
        const idsClientes = dataClientes.map((cliente) => cliente.id);
  ...
  useEffect(() => {
    if (session) {
      ...
      Promise.all([carregarDados(), carregarConfiguracao()]).finally(() => {
        setLoading(false);
      });
    } else {
      setServicos([]);
  ...
  useEffect(() => {
    const algumModalAberto =
      showModal || showClienteModal || showSolicitantesModal || showConfigModal;
  ...
  const confirmarBulkStatusUpdate = async () => {
    ...
    try {
      ...
      showToast(
        `${selectedIds.length} serviços atualizados com sucesso!`,
        "sucesso"
      );
  ...
  const salvarServico = async () => {
    try {
      if (!profileData?.consultoria_id) {
        throw new Error("Consultoria não identificada no perfil.");
      }

      const dadosParaSalvar = {
        ...formData,

        user_id: editingService ? editingService.user_id : session.user.id,
      ...
      if (!canViewFinancial) {
        delete dadosParaSalvar.valor_hora;
        delete dadosParaSalvar.valor_total;
      }
  ...
  const salvarCliente = async () => {
    ...
    try {
      ...
      if (!profileData?.consultoria_id) {
        throw new Error("Consultoria não identificada.");
      }
  ...
  const confirmInactivation = async () => {
    ...
    try {
      ...
      console.error("Erro inativar:", error);

      showToast("Erro ao inativar cliente!", "erro");
    }
  };
  ...
  const reativarCliente = async (id) => {
    try {
      ...
      showToast("Erro ao reativar cliente.", "erro");
    }
  };
  ...
  const handleGerarPDF = (semValores = false) => {
    const semValoresEfetivo = canViewFinancial ? semValores : true;

    const baseDados = servicosFiltradosData;

    const dadosParaRelatorio =
      selectedIds.length > 0
        ? baseDados.filter((servico) => selectedIds.includes(servico.id))
        : baseDados;
    ...
    const nomeParaRelatorio =
      nomeConsultor || profileData?.nome || session?.user?.email || "Consultor";

    const pdfBlob = gerarRelatorioPDF(
      dadosParaRelatorio,
      filtros,
      nomeParaRelatorio,
      semValoresEfetivo
    ...
    a.download = semValoresEfetivo
      ? "relatorio-horas-canais.pdf"
  ...
  const handleExportarExcel = (semValores = false) => {
    ...
    const nomeParaRelatorio =
      nomeConsultor || session?.user?.email || "Consultor";
  ...
  const handleEnviarEmail = async () => {
    if (!filtros.cliente || filtros.cliente.length === 0) {
      showToast("Selecione um cliente no filtro.", "erro");
    ...
    const dadosParaEnvio =
      selectedIds.length > 0
        ? servicosFiltradosData.filter((servico) =>
            selectedIds.includes(servico.id)
          )
        : servicosFiltradosData;

    if (dadosParaEnvio.length === 0) {
      showToast("Não há serviços selecionados para enviar.", "erro");
    ...
    const clienteSelecionado = clientes.find(
      (cliente) => cliente.nome === nomeClienteAlvo
    );
    ...
    try {
      ...
      dadosParaEnvio.forEach((servico) => {
        if (servico.cliente !== clienteSelecionado.nome) {
          return;
        }
        ...
        const solicitanteEncontrado = todosSolicitantes.find(
          (solicitante) =>
            solicitante.nome.trim().toLowerCase() === nomeNoServico
        );

        if (solicitanteEncontrado) {
          if (solicitanteEncontrado.coordenador_id) {
            const chefe = todosSolicitantes.find(
              (solicitante) =>
                solicitante.id === solicitanteEncontrado.coordenador_id
            );

            if (chefe && chefe.email) {
              emailDestino = chefe.email;

              if (solicitanteEncontrado.email) {
                emailCC = solicitanteEncontrado.email;
              }
            }
        ...
        if (emailCC) {
          pacotesDeEnvio[emailDestino].ccs.add(emailCC);
        }
      });

      if (Object.keys(pacotesDeEnvio).length === 0) {
        showToast("Nenhum e-mail de gestor/solicitante encontrado.", "erro");
      ...
      for (const [emailDestino, pacote] of Object.entries(pacotesDeEnvio)) {
        ...
        const response = await fetch("/api/enviar-email", {
          ...
          body: JSON.stringify({
            ...
            periodo: filtros.dataInicio
              ? `${new Date(filtros.dataInicio).toLocaleDateString()} a ...`
              : "Período Geral",
          }),
        });
      ...
      if (servicosSemDestino > 0) {
        showToast(
          `Enviados: ${enviados}. Atenção: ${servicosSemDestino} serviços sem email cadastrado.`,
          "aviso"
        );
      } else {
        showToast(
          `Sucesso! ${enviados} pacote(s) de email enviados.`,
          "sucesso"
        );
      }
    } catch (error) {
  ...
  const editarServico = (servico) => {
    ...
    setFormData({
      ...
      valor_hora: canViewFinancial ? servico.valor_hora : undefined,
  ...
  const handleDuplicateService = (servicoOriginal) => {
    setFormData({
      ...
      valor_hora: canViewFinancial ? servicoOriginal.valor_hora : undefined,
  ...
  const servicosFiltrados = () => {
    let result = servicos.filter((servico) => {
      if (filtros.canal.length > 0) {
        const nomeCanalServico = servico.canais?.nome || "Direto";

        if (!filtros.canal.includes(nomeCanalServico)) {
          return false;
        }
      }

      if (filtros.cliente.length > 0) {
        if (!filtros.cliente.includes(servico.cliente)) {
          return false;
        }
      }

      if (filtros.status.length > 0) {
        if (!filtros.status.includes(servico.status)) {
          return false;
        }
      }

      if (filtros.dataInicio && servico.data < filtros.dataInicio) {
        return false;
      }

      if (filtros.dataFim && servico.data > filtros.dataFim) {
        return false;
      }

      if (filtros.solicitantes && filtros.solicitantes.length > 0) {
        const solNome = (servico.solicitante || "").trim();

        if (!filtros.solicitantes.includes(solNome)) {
          return false;
        }
      }

      if (filtros.demanda && filtros.demanda.length > 0) {
        const nomeDemanda = servico.demandas
          ? `#${servico.demandas.codigo} - ${servico.demandas.titulo}`
          : "Sem Demanda";

        if (!filtros.demanda.includes(nomeDemanda)) {
          return false;
        }
      }

      if (filtrosConsultores.length > 0) {
        if (!filtrosConsultores.includes(servico.user_id)) {
          return false;
        }
      }
  ...
  const filtrarClientes = (todosClientes) => {
    return mostrarInativos
      ? todosClientes
      : todosClientes.filter((cliente) => cliente.ativo !== false);
  };
  ...
  // OPÇÕES DE FILTRO
  // =========================================================
  const opcoesCanais = ["Direto", ...canais.map((canal) => canal.nome)];

  const opcoesClientes = clientes.map((cliente) => cliente.nome);
  ...
  const opcoesSolicitantes = [
    ...new Set(
      listaSolicitantesGeral.map((solicitante) => solicitante.nome.trim())
    ),
  ];

  const opcoesDemandas = listaDemandas.map(
    (demanda) => `#${demanda.codigo} - ${demanda.titulo}`
  );
  ...
  // =========================================================
  const statsTabela = {
    totalHoras: servicosFiltradosData.reduce(
      (sum, servico) =>
        sum + parseFloat(servico.qtd_horas || servico.horas || 0),
      0
    ),

    totalValor: canViewFinancial
      ? servicosFiltradosData.reduce(
          (sum, servico) => sum + parseFloat(servico.valor_total || 0),
          0
        )
      : 0,
    ...
    <div className="flex h-screen bg-gray-50 dark:bg-gray-950 overflow-hidden">
        ...
        <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 h-16 flex items-center justify-between px-6 shadow-sm z-10">
          <div className="flex items-center gap-4">
            <button
              ...
              className="md:hidden p-2 -ml-2 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800"
            >
              <Menu size={24} />
            </button>
            ...
            <h1 className="text-xl font-bold text-gray-800 dark:text-white flex items-center gap-2">
              ...
              {activeTab === "admin-tenants" && isSuperAdmin && (
                <>
                  <ShieldCheck className="text-yellow-600 dark:text-yellow-400" />
          ...
          {activeTab === "servicos" && (
            <button
              ...
              className="bg-indigo-600 dark:bg-indigo-600 text-white px-4 py-2 rounded-xl flex items-center gap-2 hover:bg-indigo-700 dark:hover:bg-indigo-500 transition-all duration-200 hover:scale-105 active:scale-95 text-sm font-bold shadow-md shadow-indigo-200 dark:shadow-none"
            >
              <Plus size={18} />
          ...
          <div className="w-full max-w-full px-4 md:px-8 py-6 animate-fade-in-up pb-10">
            {loading ? (
              ...
              <>
                  ...
                  <div className="space-y-6">
                    ...
                    <div
                      className={`grid grid-cols-1 ${
                        canViewFinancial ? "md:grid-cols-3" : "md:grid-cols-2"
                      } gap-4`}
                    >
                      <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-100 dark:border-gray-700 flex items-center justify-between shadow-sm">
                        <div>
                          ...
                          <p className="text-xl font-black text-indigo-600 dark:text-indigo-400">
                            {formatHoursInt(statsTabela.totalHoras)}
                          </p>
                        </div>
                      ...
                      {canViewFinancial && (
                        <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-100 dark:border-gray-700 flex items-center justify-between shadow-sm">
                          <div>
                            ...
                            <p className="text-xl font-black text-green-600 dark:text-green-400 truncate max-w-[150px]">
                              {formatCurrency(statsTabela.totalValor)}
                            </p>
                          </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                        ...
                        <div className="col-span-full md:col-span-2 lg:col-span-2 grid grid-cols-2 gap-4">
                          <input
                            ...
                            className="border border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:text-white rounded-lg px-3 pb-2 pt-5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none w-full h-[46px]"
                          />
                      ...
                      <div className="flex justify-end gap-3 pt-2 items-center">
                        {selectedIds.length > 0 && (
                          <div className="flex items-center gap-2 mr-auto bg-indigo-50 dark:bg-indigo-900/20 px-3 py-1.5 rounded-lg border border-indigo-100 dark:border-indigo-800 animate-in fade-in slide-in-from-left-5">
                            <span className="text-xs font-bold text-indigo-700 dark:text-indigo-300 whitespace-nowrap">
                        ...
                        <button
                          ...
                          className={`flex items-center gap-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                            emailEnviando ? "opacity-50 cursor-not-allowed" : ""
                          }`}
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex gap-2 md:gap-3">
                          <button
                            ...
                            className={`px-3 md:px-4 py-2 rounded-lg text-sm font-medium border flex items-center gap-2 transition-colors ${
                              mostrarInativos
                ...
                {/* RELATÓRIOS - ADMIN/DONO/SUPER ADMIN */}
                {activeTab === "reports" && isAdmin && <Reports />}

                {/* EQUIPE - ADMIN/DONO/SUPER ADMIN */}
                {activeTab === "team" && isAdmin && (
                  <TeamManagement showToast={showToast} />
                )}
      ...
      <SolicitantesModal
        ...
        onClose={() => {
          ...
          setClienteParaSolicitantes(null);
        }}
      ...
      {/* =================================================== */}
      <div
        className={`fixed top-6 right-6 w-96 max-w-xs px-6 py-3 rounded-xl shadow-lg text-white transform transition-all duration-500 z-[90] ${
          toast.visivel
      ...
      {aviso.mensagem && (
        <div
          className={`fixed top-4 right-4 px-4 py-2 rounded shadow-lg text-white transition-all z-[90] pointer-events-auto ${
