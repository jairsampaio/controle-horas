...
const ServicesKanban = ({ servicos = [], onEditService, onStatusChange }) => {
  ...
  // MANUTENÇÃO: Se adicionar novos status no banco, adicione aqui também.
  const columns = [
  ...
  const handleDragStart = (e, serviceId) => {
    ...
    // Pequeno truque visual para o elemento arrastado ficar translúcido
  ...
  const handleDrop = async (e, targetStatus) => {
    ...
    // 1. Atualização Visual Imediata (UI Otimista)
    // Isso faz a interface parecer instantânea antes mesmo do banco responder
    const updatedServices = localServices.map(s => 
      s.id === draggedServiceId ? { ...s, status: targetStatus } : s
    );
    setLocalServices(updatedServices);

    // 2. Avisa o App.js para salvar a alteração real no banco
    if (onStatusChange) {
  ...
  const formatDate = (dateString) => {
    ...
    // Adiciona hora fixa para evitar problemas de fuso horário (-3h) ao converter
    const date = new Date(dateString + 'T00:00:00'); 
    const hoje = new Date();
    
    // Verifica se está atrasado (Data do serviço < Hoje E Status não é Pago)
    // Nota: A lógica de "não pago" deve ser tratada no render, aqui checamos apenas data
  ...
  return (
    <div className="flex h-full overflow-x-auto gap-4 pb-4 items-start animate-fade-in custom-scrollbar">
      ...
      {columns.map(col => {
        // Filtra serviços para esta coluna específica
        // Fallback: Se status for nulo/inválido, joga para 'Pendente'
              ...
              {colServices.map(service => (
                ...
                >
                  ...
                  <div className="flex justify-between items-center border-t border-gray-50 dark:border-gray-700 pt-2 mt-2">
                    {/* Exibe ícone de alerta se estiver atrasado E não pago */}
