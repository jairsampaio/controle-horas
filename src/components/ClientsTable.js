...
// MANUTENÇÃO: Função auxiliar de cores (definida fora para performance)
    ...
    <>
      ...
      <div className="md:hidden space-y-4">
        {clientes.map(cliente => (
          <div 
            key={cliente.id} 
            className={`p-4 rounded-xl shadow-sm border flex flex-col gap-3 transition-colors 
              ${cliente.ativo === false 
          ...
          >
            <div className="flex justify-between items-start">
              ...
              {/* Badge Status (Apenas se Inativo no mobile) */}
              {cliente.ativo === false && (
