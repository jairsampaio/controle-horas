...
const SolicitantesModal = ({ isOpen, onClose, cliente, userId }) => {
  ...
  // Cores dinâmicas para o cabeçalho
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 animate-fade-in">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
        <div className={`p-4 flex justify-between items-center text-white transition-colors duration-300 ${headerColor}`}>
          <div><h2 className="text-lg font-bold">{editingId ? 'Editando Pessoa' : 'Gestão de Equipe'}</h2><p className="text-xs opacity-90">{cliente?.nome}</p></div>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <form onSubmit={handleSalvar} className={`form-area p-4 rounded-lg mb-6 border transition-colors space-y-3 ${editingId ? 'bg-orange-50 border-orange-200' : 'bg-gray-50 border-gray-200'}`}>
            <h3 className={`text-sm font-bold flex items-center gap-2 ${editingId ? 'text-orange-700' : 'text-gray-700'}`}>{editingId ? <Edit2 size={16} /> : <Plus size={16} />} {editingId ? 'Editar Dados' : 'Cadastrar Pessoa'}</h3>
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button type="submit" className={`flex-1 text-white py-2 rounded text-sm font-medium flex justify-center items-center gap-2 transition-colors ${editingId ? 'bg-orange-500 hover:bg-orange-600' : 'bg-indigo-600 hover:bg-indigo-700'}`}><Save size={16} /> {editingId ? 'Salvar' : 'Cadastrar'}</button>
            </div>
          </form>
              ...
              return (
                <div key={sol.id} className={`flex justify-between items-center p-3 rounded-lg border transition-all ${isEditingItem ? 'border-orange-400 bg-orange-50 shadow-md ring-1 ring-orange-200' : 'hover:shadow-sm bg-white'}`}>
                  <div className="flex flex-col">
                    <span className="font-bold text-gray-800 flex items-center gap-2">{sol.is_coordenador ? <Shield size={14} className="text-indigo-600" /> : <User size={14} className="text-gray-400" />}{sol.nome}{sol.is_coordenador && <span className="text-[10px] bg-indigo-200 text-indigo-800 px-1.5 rounded uppercase font-bold">Coord.</span>}</span>
                    <div className="flex flex-col gap-0.5 mt-0.5 ml-0.5">{sol.email && <span className="text-xs text-gray-500 flex items-center gap-2"><Mail size={12} className="text-gray-400" /> {sol.email}</span>}{!sol.is_coordenador && nomeChefe && <span className="text-xs text-gray-400 flex items-center gap-1">↳ Responde a: <strong>{nomeChefe}</strong></span>}</div>
                  </div>
